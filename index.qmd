...
geom_pie()
injury severity x vehicle year (proportion of years that contributed to severe injuries)

Crash %>%
  group_by(Injury.Severity, Vehicle.Year) %>%
  filter(Vehicle.Year > 1900) %>%
  ggplot() +
  geom_dotplot(mapping = aes(x = Vehicle.Year, color = Injury.Severity)) 
...
Crash %>%
  ...
  filter(Weather != "NA") %>%
  ggplot() +
  geom_dotplot(mapping = aes(x = Injury.Severity, color = Weather)) 

ggplot(data = Crash) + 
  geom_point(mapping = aes(x = Injury.Severity, y = Driver.At.Fault, color = Light))
```
